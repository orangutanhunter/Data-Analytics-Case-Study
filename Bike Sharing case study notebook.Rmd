...
```{r setting_wd , echo=FALSE,results='hide'}
getwd()
...
##### Removing na values from the column gender and birthyear

Removing the na values for gender and birthyear will disproportionately remove more Customers than Subscribers since customers are one time users and would need to fill that information every time they get a ticket and they tend to more often than not leave out that information.
But since Part 1 of this case study is more focused on these two variables, it is important to drop the na values.
```{r}
trips <- trips %>% 
...
##### Creating a new variable for duration by subtracting starttime from endtime
```{r}
trips <- trips %>% 
...
###### create a variable age by subtracting the birthyear by 2019
```{r}
trips <- trips %>% 
...
creating a variable age by subtracting the birthyear by 2019
```{r}
trips <- trips %>% 
  mutate(age=2019-birthyear)
...
There are some observations in the age variable that have more than 90 years of age. The oldest age is 116. These are errors as no one of that age is physically capable of using a bike.Clearly, some people entered the wrong birthyear. We need to clean these observations.
We will set the cutoff of 90 years. There are also some outliers that need to be removed.
...
```
There are some outliers in the data that are skewing the results.  
...
print(outlier) %>% 
  ...
  kable_styling()
```
Removing the outliers as these values don't make any sense.
```{r}
trips <- trips %>% 
...
##### Correcting the variable names in trips1
```{r}
...
The following variables are not available in all the data frames.  
...
We need to drop these variables to ensure consistency across all data frames
...
##### Correcting the column names and arrangement in trips4 to ensure consistency across data frames
```{r}
...
#### Appending all 4 data frames into a single data frame
```{r}
all_trips <- rbind(trips1,trips2,trips3,trips4)
```

##### Changing the usertype observations from user/casual to customer/subscriber.  
In trips1-trips3 the usertype is customer/subscriber. While in trips4, the usertype is user/casual. 

We need to convert usertypes into the same category
```{r}
all_trips <- all_trips %>% 
...
Make new varibles for date, month, year and weekday
...
Making a variable for tripduration in minutes
...
There is an observation in tripduration with a negative value. 
Since time cannot be a negative, we need to clean this value
```{r}
all_trips <- all_trips %>% 
...
Calculating summary statistics for duration
```{r}
